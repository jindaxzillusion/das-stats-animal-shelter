...
shelter <- shelter %>% 
  mutate(animal=as.factor(animal_type),intake=as.factor(intake_type),outcome=as.factor(outcome_type),
         chip=as.factor(chip_status)) %>% 
  select(time=time_at_shelter,animal,intake,outcome,chip,month,year)
...
```{r ggpairs}
g1 <- shelter %>% select(-c(month,year)) %>% ggpairs(aes(fill=animal))
g1$nrow <- 1
...
From the exploratory analysis, it seemed fitting to try models based on intake and/or outcome, and then build up from there. Having fitted some models, model1 seems to be the best, which is simply intake as the only explantory variable for the log-odds.
